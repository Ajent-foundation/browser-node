...
export async function cdp(
    ...
    try {
        ...
        let query = "SELECT * FROM cdp_events WHERE sessionId = ?";
        ...
        next(await buildResponse(200, { events: result }));
    } catch (error) {
        LOGGER.error("Failed to get CDP events", { error });
        next(await buildResponse(500, { error: "Failed to get CDP events" }));
    }
}
